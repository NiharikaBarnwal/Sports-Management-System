...
<br>
<h2 class="text-center"><b>Athlete Modification</b></h2><hr>
...
<?php
    while($result_array_row = mysqli_fetch_array($result)) {
        ...
        $query_str1 = "select group_concat(s.name) ath_sport from athlete_sport a_s, sport s 
                            where s.sport_id=a_s.sport_id and a_s.ath_id=".$result_array_row['ath_id'];
        $result1 = mysqli_query($connection,$query_str1);
        $result_array = mysqli_fetch_array($result1);
        ...
        echo '<td>'.$result_array['ath_sport'].'</td>';?>
        <td><img class="img-thumbnail" src="upload_img/<?php echo $result_array_row['ath_img']?>" ></td><?php
        echo '<td><a href=ath_form.php?ath_id='.$result_array_row['ath_id'].'>Edit</a></td>';
        echo '<td><a href=delete_ath.php?ath_id='.$result_array_row['ath_id'].'>Delete</a></td>';
        echo '</tr>';
    }
